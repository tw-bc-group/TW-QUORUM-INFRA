...
CONFIG_TMPL=$(cat ${DDIR}/{{ include "quorum.name" . }}-tessera-config${TESSERA_CONFIG_TYPE}.json.tmpl);

echo  \" sed \"s/%QUORUM-{{ include "quorum.name" . }}_SERVICE_HOST%/$QUORUM_{{ include "quorum.name" . }}_SERVICE_HOST/g\" \";
CONFIG_WITH_OTHER_HOSTS=$(echo $CONFIG_TMPL |  sed \"s/%QUORUM-{{ include "quorum.name" . }}_SERVICE_HOST%/$QUORUM_{{ include "quorum.name" . }}_SERVICE_HOST/g\" ) ;
CONFIG_WITH_ALL_HOSTS=$(echo $CONFIG_WITH_OTHER_HOSTS | sed \"s/%THIS_SERVICE_HOST%/$QUORUM_{{ include "quorum.name" . }}_SERVICE_HOST/g\");
